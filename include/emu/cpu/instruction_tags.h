...
namespace detail { struct instruction_tag {}; }

/** CPU instruction tag type. */
template<class InstrTag>
concept instruction_tag =
    std::derived_from<InstrTag, detail::instruction_tag> &&
    std::same_as<decltype(InstrTag::name), char const* const>;

/** Instruction tags. */
namespace instruction_tags {

/** Defines a tag for a CPU instruction. */
#define EMU_DEFINE_INSTRUCTION_TAG(InstrTag)                            \
    struct InstrTag : detail::instruction_tag {                         \
        static constexpr char const* name = #InstrTag;                  \
    }
