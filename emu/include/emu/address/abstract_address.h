...
public:
    ...
    /** Constructs an address from a page index and an offset within that page. */
...
struct std::hash<Address> {
    std::size_t operator()(emu::abstract_address addr) const noexcept {
        constexpr std::hash<typename Address::underlying_type> hash;
        return hash(addr.to_uint());
    }
};
